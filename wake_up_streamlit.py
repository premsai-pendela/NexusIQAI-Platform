...
BROWSER_PAGELOAD_TIMEOUT_SECONDS = float(os.getenv("BROWSER_PAGELOAD_TIMEOUT_SECONDS", "5"))
...
CHROME_BINARY = os.getenv("CHROME_BINARY", "/usr/bin/chromium-browser").strip()
...
# FIX: CSS data-testid selectors are primary (stable, don't depend on button text).
# XPath is last-resort fallback only.
...
def click_wake_button_if_available(driver) -> bool:
    ...
    try:
        ...
        # FIX: JS click as retry if normal click fails (e.g. element covered)
...
async def main() -> None:
    with open(LOG_FILE, "a", encoding="utf-8") as log_file:
        ...
        log_message(log_file, f"Configured wake interval: every {WAKE_INTERVAL_HOURS} hour(s)")
        log_message(log_file, f"Configured concurrency: up to {min(MAX_CONCURRENT_APPS, len(UNIQUE_STREAMLIT_APPS))} app(s)")

        if not should_run_interval(log_file):
            log_message(log_file, "Execution finished")
            return

        summary = {"awake": 0, "woken": 0, "woken_unverified": 0, "errors": 0}
        ...
        for task in asyncio.as_completed(tasks):
            ...
            if state == "awake":
                log_message(log_file, f"App already awake: {url} ({detail})")
            elif state == "woken":
                log_message(log_file, f"App woken and confirmed: {url} ({detail})")
            elif state == "woken_unverified":
                log_message(log_file, f"App wake clicked, load unconfirmed: {url} ({detail})")
            else:
